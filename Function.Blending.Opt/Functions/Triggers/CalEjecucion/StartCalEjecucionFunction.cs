...
public sealed class StartCalEjecucionFunction(IMediator mediator, IProblemDetailsWriter problem, IRequestContext requestContext, IFunctionContextAccessor fctxAccessor, ISysLogService syslog)
{
  ...
  {
    // 1) Leer JSON
    var dto = await req.TryReadJsonWithSysLogAsync<StartCalEjecucionRequest>(fctx, requestContext, fctxAccessor, syslog);
    if (dto is null)
    {
      return await problem.CreateAsync(
          fctx, req, HttpStatusCode.BadRequest,
          type: "urn:blending:error:invalid-json",
          title: "Bad Request",
