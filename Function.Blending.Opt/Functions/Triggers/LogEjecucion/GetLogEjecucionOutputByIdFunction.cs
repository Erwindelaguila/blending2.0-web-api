using Function.Blending.Opt.Application.Features.LogEjecucion.Queries.GetOutputById;
...
public sealed class GetLogEjecucionOutputByIdFunction(
  IMediator mediator,
  IProblemDetailsWriter problem,
  IRequestContext ctx)
{
  [Function(nameof(GetLogEjecucionOutputByIdFunction))]
  [RequireScopes(ConfigurationKeys.Auth.Scopes.Logistics.ReadOutputById)]
  public async Task<HttpResponseData> Run(
      [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = FunctionRoutes.Logistics.GetOutputById)]
      HttpRequestData req,
      ...
      Guid id)
  {
    ...
    var result = await mediator.Send(new GetLogEjecucionOutputByIdQuery(id, convertDates, tzId));
