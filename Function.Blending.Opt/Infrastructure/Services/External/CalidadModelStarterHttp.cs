...
public sealed class CalidadModelStarterHttp : ICalidadModelStarter
{
  private readonly HttpClient _http;
  private readonly ILogger<CalidadModelStarterHttp> _logger;
  private readonly IOptions<CalidadModelOptions> _opts;

  public CalidadModelStarterHttp(
    HttpClient http,
    IOptions<CalidadModelOptions> opts,
    ILogger<CalidadModelStarterHttp> logger)
  {
    _http = http;
    _opts = opts;
    _logger = logger;
  ...
  public Task<DispatchResult> StartAsync(CalidadModelPayload payload, CancellationToken ct)
  {
