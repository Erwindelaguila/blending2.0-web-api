...
) : ICalEjecucionRepository
{
  ...
  // GetById
  // ============================
  public async Task<DE.CalEjecucion?> GetByIdAsync(Guid id, CancellationToken ct)
  {
    var model = await db.Set<EF.CalEjecucion>()
                        .AsNoTracking()
                        .FirstOrDefaultAsync(x => x.Id == id, ct);

    return model is null ? null : mapper.Map<DE.CalEjecucion>(model);
  }
  ...
  // ============================
  public async Task<DE.CalEjecucion?> StartAsync(
    Guid plantaId,
  ...
  )
  {
    ...
    db.Set<EF.CalEjecucion>().Add(model);
    ...
    var exists = await db.Set<EF.CalEjecucion>()
                         .AsNoTracking()
    ...
    return mapper.Map<DE.CalEjecucion>(model);
  }

  // ============================
  // Complete (Output + estado)
  // ============================
  public async Task<DE.CalEjecucion?> CompleteAsync(
    Guid id,
  ...
  {
    var set = db.Set<EF.CalEjecucion>();
    var model = await set.FindAsync([id], ct);
    if (model is null) return null;

    using var tx = await db.Database.BeginTransactionAsync(ct);

    var outRes = await db.Set<EF.CalOutResumen>().AsNoTracking().Where(x => x.EjecucionId == id).ToListAsync(ct);
    var outDet = await db.Set<EF.CalOutDetalle>().AsNoTracking().Where(x => x.EjecucionId == id).ToListAsync(ct);

    // ¿Ya existen outputs? (idempotencia)
    var outResExists = await db.Set<EF.CalOutResumen>().AsNoTracking().AnyAsync(x => x.EjecucionId == id, ct);
    var outDetExists = await db.Set<EF.CalOutDetalle>().AsNoTracking().AnyAsync(x => x.EjecucionId == id, ct);
  ...
  public async Task<bool?> SetAceptadoAsync(Guid id, IReadOnlyList<Guid>? grupos, Guid modificadoPorId, CancellationToken ct)
  {
    ...
    (List<EF.CalOutResumen> resumenesYaNoAceptados, List<EF.CalOutResumen> nuevosResumenesAceptados) = await CalidadOutputUpdater.UpdaterResumenesAsync(db, id, grupos, modificadoPorId, ct);
