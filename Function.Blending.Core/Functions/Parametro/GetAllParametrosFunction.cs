...
public class GetAllParametrosFunction
{
    ...
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, HttpMethods.Get, Route = ApiRoutes.Core.Parametro.Base)]
        HttpRequestData req)
    {
        try
        {
            ...
            var isLogistic = query.Get("isLogistic") ?? "0";
            if (isLogistic != "0" && isLogistic != "1")
            {
                throw new ArgumentException("El parámetro 'isHarina' debe ser '0' o '1'.");
            }

            bool utilLogistic = isLogistic == "1";
            ...
            var queryRequest = new GetAllParametrosQuery(page, size, filtersToApply,utilLogistic);
            var result = await _mediator.Send(queryRequest);

            if (utilLogistic)
            {
                return await HttpResponseHelper.WriteBaseResponseAsync(req,
                    BaseResponse<List<ParametroShortDTO>>.Success(result.ParametroShortList, "Parámetros obtenidos correctamente"));
            }

            return await HttpResponseHelper.WriteBaseResponseAsync(req,
                BaseResponse<PagedResponse<ParametroDTO>>.Success(result.ParametroPaginate, "Parámetros obtenidos correctamente"));
        }
